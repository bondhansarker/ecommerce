...
func (productRepo productRepository) GetRecords(ctx context.Context, productFilterParams V1Domains.ProductFilterParams, currentPageInt int, itemPerPageInt int) (outputDomains []V1Domains.ProductDomain, paginationResult *helpers.PaginationResult, err error) {
	...
	if len(productRecords) > 0 {
		totalCount = productRecords[0].TotalCount
	}
	paginationResult, err = helpers.GetPaginationInfoData(totalCount, currentPageInt, itemPerPageInt)

	if err != nil {
		return nil, nil, err
	}
	return records.ToArrayOfProductDomain(&productRecords), paginationResult, nil
}
